...
rho.ori <- sim.dat.seed %>%
  group_by(id,dtname) %>%
  mutate(norm = scale(val, center = TRUE,scale = TRUE)) %>%
  ungroup() %>%
...
repeat{  
  ...
  sim.dat <- sim.dat.seed %>%
    ...
    mutate(
      ...
      # This condition is for data of which
      # the timepoint:0 (Control) is always 0 
    ) %>%
    dplyr::select(id,dtname,var,val) %>%
  ...
  sim.dat.rho <- sim.dat %>%              # a data.frame of the Spearman's Rhos
  ...
  if(length(rho.which) > 0){
    ...
    print(sim.dat.set)
    print(
      plyr::count(sim.dat.set,rho.which)[,2]
      )
    if(min(plyr::count(sim.dat.set,'rho.which')[,2]
