#R --vanilla --quiet < Morimoto_TDC.2016.main_20160317_args.R --args Selevsek_2015_all_Selevsek_20160123.RData             out.setting=F clustering=F ScaleBoot=F  lag.neg.ign=T  Selevsek_2015_id.convert_all.csv             > Selevsek_2015.all_Qian.log 2>&1 &
#R --vanilla --quiet < Morimoto_TDC.2016.main_20160317_args.R --args Selevsek_2015_pentose_Selevsek_20160123.RData         out.setting=F clustering=F ScaleBoot=F  lag.neg.ign=T  Selevsek_2015_id.convert_pentose.csv         > Selevsek_2015.pentose_noHCA.log 2>&1 &
#R --vanilla --quiet < Morimoto_TDC.2016.main_20160317_args.R --args Selevsek_2015_GlySerThrMetabo_Selevsek_20160123.RData out.setting=F clustering=F ScaleBoot=F  lag.neg.ign=T  Selevsek_2015_id.convert_GlySerThrMetabo.csv > Selevsek_2015.GlySerThrMetabo_noHCA.log 2>&1 &
#R --vanilla --quiet < Morimoto_TDC.2016.main_20160317_args.R --args Fournier_2010_all_Fournier_20160123.RData             out.setting=F clustering=F ScaleBoot=F  lag.neg.ign=T  Fournier_2010_id.convert_all.csv             > Fournier_2010.all_Qian.log 2>&1 &

# This source code is built under R ver.3.1.2

c_args <- commandArgs(trailingOnly=T)
param.setting     <- c_args[2]
param.setting <- 'out.setting=T'
...
line.plot <- function(data){
  ...
  plot( p+ plot.type+ plot.theme + plot.title + plot.ylim)
  }
...
#  shape_basket <- c(8,15,16,17,18,19)
  shape_basket <- c(9,10,11,12,13,14) 
  id <- data$id[1]
  accession <- data$accession[1]
  delay <- data$delay[1]
  edge  <- data$edges[1]
  ...
  if(length(unique(data$id))<=6){
    ...
    plot.shape <- scale_shape_manual(values=rep(16,length(unique(data$id))))
  }
...
pdf(
  ...
  scat.plot.list <- dlply(
    annMstLong %>%
      rename(raw.prt=prt,raw.trc=trc)%>%
      rename(prt=prt.norm,trc=trc.norm)%>%
      mutate(delay='disabled',var.new=factor(var)), .(edges), scat.plot) 
