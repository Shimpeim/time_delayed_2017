...
gq_method <- function(vec_i,vec_j,timepoint,rep,pos_neg){
  gq   <- list()
  for(rep in 1:rep){
    vec_i <- c(append(
...
gq_method_M_mat <- function(vec_i,vec_j,timepoint,rep,pos_neg){
  gq   <- list()
  for(rep in 1:rep){
    vec_i <- c(append(
      c(vec_i),0,after=((timepoint+1)*(rep-1))
  ...
  for(i in 2:length(veci)){
    for(j in 2:length(vecj)){
      ...
      if(pos_neg == 'pos'){mat[i,j] <-  as.numeric(veci[i])*as.numeric(vecj[j]) }
      if(pos_neg == 'neg'){mat[i,j] <-  as.numeric(veci[i])*as.numeric(vecj[j]) }
